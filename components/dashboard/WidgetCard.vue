...
const filteredTableItems = computed(() => {
    if (page.value.serverItems && page.value.serverItems.length > 0) {
        ...
        return (
            ...
            (item.is_active && item.is_active.toString().includes(searchItem.value)) ||
            (item.description && item.description.toLowerCase().includes(searchItem.value.value.toLowerCase()))
        );
        });
    }
    return [];
...
function processData(data: Record<string, any>[], headers: any[]): { graphics: Record<number, GraphicData[][]>, tables: Record<number, TableData[]> } {
  ...
  data.forEach((item, index) => {
    ...
    Object.keys(item).forEach(key => {
      ...
      if (headerInfo) {
        const dataItem = { key: headerInfo.text, value, headers: headerInfo };
...
watchEffect(() => {
  if (rows.value.length > 0) {
    const { graphics: newGraphics, tables: newTables } = processData(rows.value, props.dataTest.headers);
                    ...
                    <v-col cols="12">
                        <v-card>
                            <v-data-table-server
                                ...
                                :items="searchItem.value ? filteredTableItems : paginatedItems"
